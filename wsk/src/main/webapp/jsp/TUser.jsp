...
<script type="text/javascript">
    ...
    <!--如果不需要回显第一个参数传入""，如果不需要设定disabled属性传入FALSE-->
        ...
        $("#addform_save_btn").off('click').on('click', function() {
            ...
            var url = "";
            if("1" == hiddenType){
                url = baseURL + "/wsk/tUser/save";
            }else if ("2" == hiddenType){
                url = baseURL + "/wsk/tUser/update";
            ...
            if(selectRocords.length <= 0){
                ...
                bootbox.confirm("选择了"+selectRocords.length+"条, 确认删除?", function(truthBeTold){
                    if(truthBeTold){
                        $.ajax({
                            type: "POST",
                            url: baseURL + "/wsk/tUser/deleteByUuid",
                            data:{
                                uuids:uuids
                            },
                            contentType: "application/x-www-form-urlencoded;charset=utf-8",
    ...
    function addOrUpdate(type,record){
        if("add" == type){
            ...
            $.ajax({
                type : "POST",
                url : baseURL + "/wsk/tUser/getDetail",
                data :param,
                contentType: "application/x-www-form-urlencoded;charset=utf-8",
                dataType : "json",
                success:function(data){
    ...
    var queryInner_Path = baseURL + "/wsk/tUser/query";
    //实现查询按钮方法
    function reloadGrid(){
        $('#queryMecGrid > tbody').empty();
        if(limit==0) {
    ...
    function daysBetween(DateOne,DateTwo)
    {
                ...
                if (file.lastIndexOf('.') != -1) {
                    ...
                    if(ext =="xlsx"){
                        ...
                        $("#importOpenWinform").ajaxSubmit({
                            type: 'post',
                            url : baseURL +'/wsk/tUser/importExcel',
                            dataType: "json",
                            success: function(data){
        ...
        $("#errorImExcel").off('click').on('click', function() {
            ...
            window.open(baseURL + "/wsk/tUser/getFailExport");
            $("#errorImExcel").modal('hide');
        });
        //下载模板按钮
        $("#download_template_btn").on('click', function() {
            window.open(baseURL +'/wsk/tUser/downloadTemplate');
        });
        ...
        $("#isExport_btn").on('click', function() {
            ...
            $("#exportform_save_btn").off('click').on('click', function() {
                window.open(baseURL +"/wsk/tUser/export");
