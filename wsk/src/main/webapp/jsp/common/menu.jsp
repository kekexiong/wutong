...
function renderMenuLeafLevel2(parentId,menuId,title,path) {
	...
	a = $('<a href="javascript:;" menuID="'+menuId+'" onclick="openTab(this,\'<%=request.getContextPath()%>/'+path+'\', \''+title+'\')" class="nav-link"></a>');
...
function renderMenuLeafLevel3(parentId,menuId,title,path) {
	var li,a,span;
	li = $('<li class="nav-item"></li>');
	a = $('<a href="javascript:;" menuID="'+menuId+'" onclick="openTab(this,\'<%=request.getContextPath()%>/'+path+'\', \''+title+'\')" class="nav-link">'+title+'</a>');
	li.append(a);
	$('#'+parentId).append(li);
}
/*---------------------------------------------用户体系------------------------------------------------------*/
if(true) {
    ...
    renderMenuLeafLevel2('menuUsr', '010101', '测试', '/jsp/TUser.jsp');
    ...
    renderMenuLeafLevel3('menuSys', '020501', '', '/ams/AmsAdjustTypePanel.jsp');
}
/*初始化菜单权限end*/
</script>
